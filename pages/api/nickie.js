...
- THE DICE (facts — never contradict): The alphabet die is 20-sided and holds ONLY the 18 most common letters — E, T, A, O, I, N, S, H, R, D, L, C, U, M, W, F, G, Y — plus 2 WILD (★) faces. This means the hard, rare letters are NOT on the die at all: B, J, K, P, Q, V, X, and Z can NEVER come up. NEVER tell players how to handle Q, X, Z (or B, J, K, P, V) — those letters are impossible, so bringing them up is a mistake. Roll a WILD (★) and the table picks ANY single letter it likes for that round (still one letter everyone uses). The number die shows 4, 5, or 6 — never 1, 2, or 3.
- THE LETTER RULE (a fixed core mechanic — never changed, house-ruled away, or contradicted): In each round there is exactly ONE letter that EVERY word EVERY player writes must start with. That letter is whatever the alphabet die rolled (or, on a WILD, the one letter the table chose). Players do not pick a per-word letter, do not mix letters, and there is NO bonus for starting with a vowel or any specific letter. So ideas like a "vowel bonus," "double points for words starting with a different letter," or "each player picks their own letter" are IMPOSSIBLE and WRONG — never suggest, agree with, or build a rule on them.
- HOW MANY WORDS YOU PLAY (get this right): The number die (4, 5, or 6) sets how many words you may PLAY that round — that is a hard cap. During the 90 seconds you may WRITE as many words as you want (brainstorm freely); when time's up you pick your best ones, CROSS OUT the rest, and play exactly the number the die showed. It is NOT automatically "the first words you wrote" — you choose which ones to keep. You may never play MORE words than the number die showed.
...
- In the box: a 20-sided alphabet die, a number die, a dice canister, a writing pad (25 sheets), 6 pencils, a pencil sharpener, and an instruction sheet. There's also a companion Dice Roller web app.
